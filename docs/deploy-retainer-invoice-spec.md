...
## ⚠️ Status check before executing (2026-04-23 verification)

The "Already shipped this session" section below was **not found in the repo** when this file was saved. Specifically:

- `apps/api/prisma/schema.prisma` has **no** `scopeSummary`, `deliverables`, `approvalTerms`, or `requirements` fields on `Retainer`.
- No `WorkspaceHubSpotSettings` model exists.
- No migration named `20260423110000_add_retainer_scope_and_hubspot_settings` exists under `apps/api/prisma/migrations/`.
- No `GET`/`PUT` `/api/workspace/hubspot-settings` handlers or route registrations in `app.ts` / `server.ts`.
- No `WorkspaceSettings.tsx` "HubSpot Partner Invite" section in `apps/web/app/components/`.

Treat the "Already shipped" section as **Phase 0** — do it first, then proceed to Phase 1.

---

## Already shipped this session

> **Verification note:** see the status check above — these were NOT in the repo when this file was saved. Do them as Phase 0 before Phase 1.
...
> Repo note (2026-04-23): a `PATCH /api/projects/:projectId/retainer` handler already exists in `apps/api/src/app.ts:2507` — reuse or extend rather than duplicating.
...
> Repo note (2026-04-23): `POST /api/invoices`, `GET /api/invoices`, `PATCH /api/invoices/:invoiceId` already exist (`apps/api/src/app.ts:4407` onwards), plus `createInvoiceRecord` / `listInvoices` / `updateInvoiceRecord` in `apps/api/src/billing.ts`. Client-scoped wrappers may still be needed; extend existing functions rather than rebuilding.
...
1. **Set the HubSpot partner invite URL** (requires Phase 0 shipped):
