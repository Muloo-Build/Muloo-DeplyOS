...
- **Public workbook share (Typeform-style)**: `DiscoveryEvidence` extended with `publicShareToken @unique`, `publicShareEnabled`, `publicShareExpiresAt`. New `WorkbookPublicSubmission` model captures anonymous submissions (firstName, lastName, email, organisation?, responses Json, status pending_review|approved|archived, reviewerNotes?). Operator routes (under `/api/projects/*` internalAuth): `PATCH /workbooks/:id/public-share`, `GET/PATCH /workbooks/:id/public-submissions[/:subId]`. Public unauth routes: `GET/POST /api/workbooks/public/:token[/submissions]` — token is the credential, server enforces enabled+expiry. Public page at `/w/[token]` (`PublicWorkbookClient.tsx`) renders contact + sanitised questions. Operator UI lives in `WorkbookPublicSharePanel.tsx` embedded in `ProjectWorkbooksPanel`.
